...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:  # we do not use this
            ...
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=x_start, x_t=x_t, t=t
                )[0],  # we do not use this
                ModelMeanType.START_X: x_start,
                ModelMeanType.EPSILON: noise,  # we do not use this
            }[self.model_mean_type]
            assert model_output.shape == target.shape == x_start.shape  # [bs, seq_len, nfeats]
